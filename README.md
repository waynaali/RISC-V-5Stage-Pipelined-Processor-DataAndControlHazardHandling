...
The processor fetches instructions through the instruction cache and performs load/store operations through the data cache. Both caches communicate with the AXI4 master interface, which connects to a behavioral AXI4 RAM model.

The verification environment checks processor execution, register writeback, store transactions, AXI4 read/write activity, and protocol control behavior.
...
Add all RTL files from the `srcs` directory.

### **2. Add the testbench files**

Add all testbench files from the `tb` directory.

### **3. Set the simulation top**

Set `tb_risc` as the simulation top module.

### **4. Set the project directory**

Set the project directory path in the Vivado TCL console.
...
Check the console output for the final verification summary.
